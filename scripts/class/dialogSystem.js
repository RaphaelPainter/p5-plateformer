...
class DialogSystem {
    ...
    isLastLine() {
        if (
            this.index >= this.dialog.lines.length - 1 &&
    ...
    draw(playerPos, image) {
        ...
        this.textBackground_y_coef =
            playerPos.y < image.height * UP_DOWN_TRESHOLD
                ? TEXT_BACKGROUND_COEF_Y_DOWN
                : this.textBackground_y_coef
        this.textBackground_y_coef =
            playerPos.y > image.height * (1 - UP_DOWN_TRESHOLD)
                ? TEXT_BACKGROUND_COEF_Y_UP
                : this.textBackground_y_coef
        ...
        if (
            this.dialog &&
            this.index != STARTING_INDEX &&
            this.dialog.lines[this.index] !== ''
        ) {
            strokeWeight(STROKE_WEIGHT)
            fill(TEXT_BACKGROUND_COLOR)
            rect(
                TEXT_BACKGROUND_X,
                this.textBackground_y,
            ...
            text(
                this.dialog.lines[this.index].substring(0, this.charIndex),
                this.text_x,
                this.text_y
            )
        }
